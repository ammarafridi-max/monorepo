...
export default function PageHero({ title, subtitle, paths = [] }) {
  ...
  return (
    <>
      ...
      <section className="bg-linear-to-b from-sand-100 to-sand-200 py-16 md:py-20">
        <Container>
          {paths.length > 0 && (
            <nav
              aria-label="Breadcrumb"
              className="mb-6 flex flex-wrap items-center gap-1.5 text-sm text-ink-mute"
            >
              {paths.map((p, i) => (
          ...
          {subtitle && (
            <p className="mt-4 max-w-2xl text-lead text-ink-soft leading-relaxed">
              {subtitle}
            </p>
          )}
        </Container>
