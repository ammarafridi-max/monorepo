...
export async function generateMetadata({ params }) {
  ...
  const description = clampDescription(
    profile.bio || `Travel insurance guides written by ${author.name} for Travl.`,
  );
...
export default async function Page({ params }) {
  ...
  const profile = author.authorProfile || {};
  const canonical = `${SITE_URL}/authors/${slug}`;
  const description = profile.bio || `Travel insurance guides written by ${author.name} for Travl.`;
