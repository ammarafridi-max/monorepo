...
export async function generateMetadata({ params }) {
  ...
  const description = metaDescriptionFrom(
    profile.bio,
    `Dummy ticket and visa guides written by ${author.name} for Dummy Ticket 365.`,
  );
...
export default async function Page({ params }) {
  ...
  const canonical = `${SITE_URL}/authors/${slug}`;
  const description =
    profile.bio || `Dummy ticket and visa guides written by ${author.name} for Dummy Ticket 365.`;
