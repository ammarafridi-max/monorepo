...
export async function confirmInsurancePaymentApi(
  sessionId,
  paymentSyncToken,
  paymentStatus = "PAID",
) {
...
export async function downloadInsurancePolicyApi(policyOrPayload, index = 0) {
  const policyId =
    typeof policyOrPayload === "object"
      ? policyOrPayload?.policyId
      : policyOrPayload;
  const docIndex =
    typeof policyOrPayload === "object" ? (policyOrPayload?.index ?? 0) : index;
...
export async function updateInsuranceApplicationApi({
  sessionId,
  paymentStatus,
}) {
  return await apiFetch(`${URL}/${sessionId}`, {
