...
async function seedEligibleApp(sys, opts = {}) {
  ...
  const email = customerEmail || `${ref.toLowerCase()}@verify.travl.ae`;
...
try {
  ...
  if (!customerEmail) r.info('VERIFY_EMAIL_TO not set — customer reminders go to a synthetic @verify.travl.ae address (Brevo will accept then bounce).');
