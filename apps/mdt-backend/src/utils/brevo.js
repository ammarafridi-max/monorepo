...
export const createContact = async ({ firstName, lastName, email }) => {
  if (!hasBrevoConfig()) {
    logger.warn(
      "Brevo createContact skipped because BREVO_API_KEY is missing",
      { email },
    );
    return false;
  }
...
export const updateContactAttribute = async ({ email, attribute, value }) => {
  if (!hasBrevoConfig()) {
    logger.warn(
      "Brevo updateContactAttribute skipped because BREVO_API_KEY is missing",
      { email, attribute },
    );
    return false;
  }
  ...
  const res = await fetch(
    `${BREVO_URL}/contacts/${encodeURIComponent(email.toLowerCase())}`,
    {
      method: "PUT",
      headers: getHeaders(),
