/**
 * JSON-LD structured-data + metadata builders. Ported from the shared frontend
 * utils used across the monorepo: a generic `createSchemaBuilders` factory plus the
 * standalone `buildFAQPage` / `buildBreadcrumbList` / `buildGraph` / `buildMetadata`
 * helpers, then configured for Picturesk at the bottom and re-exported for use
 * throughout the app (pages call the configured builders and drop the result into a
...
const _buildBreadcrumbList = ({ paths = [], baseUrl = '', basePath = '' } = {}) => ({
...
// --- Picturesk configuration (mirrors the per-app schema wrapper in the monorepo) ---
export { SITE_URL };
