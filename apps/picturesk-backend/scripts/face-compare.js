 ...
 *
 * ENV/CREDS (pass --env-file, same vars as the worker):
...
const OUT_DIR = resolve(here, 'results/face-compare');
...
async function main() {
  ...
  if (!process.env.REPLICATE_API_TOKEN) throw new Error('REPLICATE_API_TOKEN is required (pass --env-file).');
