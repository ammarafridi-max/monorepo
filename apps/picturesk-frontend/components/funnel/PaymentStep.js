...
export default function PaymentStep({ product }) {
  const paths = funnelPaths(product);
  const cfg = productConfig(product);
  const catalog = catalogFor(product);
  const LOOK_LABEL = Object.fromEntries(catalog.looks.map((l) => [l.id, l.label]));
  const ATTIRE_LABEL = Object.fromEntries(catalog.attire.map((a) => [a.id, a.label]));
  ...
  useEffect(() => {
    ...
    if (s.looks.length === 0 || s.attire.length === 0 || !s.gender || !s.ageRange)
      return router.replace(paths.select);
    if (s.images.length === 0) return router.replace(paths.upload);
    setState(s);
    track(EVENTS.PAYMENT_VIEW, { product: cfg.id });
  }, [router]);
  ...
  async function onPay() {
    ...
    setError('');
    setFailures([]);
    track(EVENTS.CHECKOUT_STARTED, { product: cfg.id });
    try {
  ...
  return (
    <section>
      ...
      <dl className="review">
        <div className="review__row">
          ...
          <dd className="review__v">
            {selectedTier.label}, {selectedTier.deliverCount} {cfg.noun}, {usd(selectedTier.priceCents)}
          </dd>
        </div>
        <div className="review__row">
          <dt className="review__k">{cfg.lookLabel}</dt>
          <dd className="review__v">{state.looks.map((id) => LOOK_LABEL[id]).filter(Boolean).join(', ')}</dd>
        </div>
        <div className="review__row">
          <dt className="review__k">{cfg.attireLabel}</dt>
      ...
      <p className="formnote" style={{ textAlign: 'left' }}>
        <strong>Look-like-you guarantee.</strong> If your {cfg.noun} don&apos;t look like you,
      ...
      {failures.length > 0 && (
        <div className="gatefail">
          ...
          <p className="formnote" style={{ textAlign: 'left', marginTop: 10 }}>
            <Link href={paths.upload}>Back to your photos</Link> to replace them.
          </p>
        </div>
      )}
      <p className="formnote" style={{ textAlign: 'left' }}>
