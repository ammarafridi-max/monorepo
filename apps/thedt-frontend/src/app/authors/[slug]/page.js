...
// The bio runs to several paragraphs. A meta description has to be one short
// sentence, so take the opening paragraph and cut it on a word boundary.
...
export async function generateMetadata({ params }) {
  ...
  const description = metaDescriptionFrom(
    profile.bio,
    `Dummy ticket and visa guides written by ${author.name} for The Dummy Ticket AE.`,
  );
...
export default async function Page({ params }) {
  ...
  const canonical = `${SITE_URL}/authors/${slug}`;
  const description =
    profile.bio || `Dummy ticket and visa guides written by ${author.name} for The Dummy Ticket AE.`;
