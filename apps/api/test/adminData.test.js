...
before(async () => {
  ...
  await AdminUser.create([
    { name: 'Admin', username: 'adminuser1', email: 'admin@picturesk.ai', passwordHash: await hashPassword('supersecret'), role: 'admin', status: 'ACTIVE' },
    { name: 'Support', username: 'supportone', email: 'support@picturesk.ai', passwordHash: await hashPassword('supersecret'), role: 'support', status: 'ACTIVE' },
  ]);
  ...
  function adminGuard(req, res, next) {
    if (adminAuthorized(req)) {
      req.user = { role: 'admin', email: 'admin-token' };
      return next();
    }
