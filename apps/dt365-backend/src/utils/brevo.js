...
export const createContact = async ({ firstName, lastName, email }) => {
  ...
  if (!res.ok) { await logBrevoFailure('createContact', res); throw new Error('Brevo sync failed'); }
  return true;
};
...
export const addContactToReviewList = async ({ email, firstName, listId }) => {
  if (!hasBrevoConfig()) {
    logger.warn('Brevo addContactToReviewList skipped because BREVO_API_KEY is missing', { email });
    return false;
  }
  ...
  if (!res.ok) { await logBrevoFailure('addContactToReviewList', res); throw new Error('Brevo addContactToReviewList failed'); }
  return true;
};
...
export const updateContactAttribute = async ({ email, attribute, value }) => {
  ...
  if (!res.ok) { await logBrevoFailure('updateContactAttribute', res); throw new Error('Brevo updateContactAttribute failed'); }
  return true;
};
