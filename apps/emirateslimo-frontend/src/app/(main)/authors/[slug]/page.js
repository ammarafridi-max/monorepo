...
export async function generateMetadata({ params }) {
  ...
  const description = clampDescription(
    profile.bio || `Travel guides written by ${author.name} for Emirates Limo.`,
  );
  ...
  return {
    ...
    openGraph: {
      ...
      title,
      description,
      images: [profile.avatarUrl || `${SITE_URL}/hero-bg.webp`],
    },
  };
}
...
export default async function Page({ params }) {
  ...
  const profile = author.authorProfile || {};
  const canonical = `${SITE_URL}/authors/${slug}`;
  const description = profile.bio || `Travel guides written by ${author.name} for Emirates Limo.`;
