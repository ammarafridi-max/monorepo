...
- --bone:   #FAF9F6   (off-white, primary background — never pure #FFFFFF)
- --cobalt: #2F5BFF   (single accent: CTAs, links, focus states ONLY)
- --ash:    #6B6B70   (muted text, captions, metadata)
- --line:   #E6E4DF   (hairline borders, dividers)
- --ok:     #1F9D57   (success/delivered)
- --warn:   #C4462B   (error/failed/refund)

Rule: one accent only. Cobalt is a scalpel, not a paintbrush. If a screen has more
than a few cobalt elements, it's wrong.

## Typography
- Display/headlines: a sharp serif (e.g. Fraunces or Instrument Serif) — editorial,
  premium, signals "photography studio".
- UI/body: a clean grotesque (e.g. Inter or Geist).
