 ...
 */
export function createInsuranceService({ InsuranceApplication, Affiliate, wis, brevo, logger, notifications }) {
  ...
  const validateCreateBody = (body) => {
    ...
    if (!body.mobile?.digits)           throw new AppError('Phone number is required', 400);
  ...
  const serializeApplicationForClient = (application, syncStatus) => {
    ...
    const payload = typeof application.toObject === 'function'
  ...
  const confirmDirectPayInsurance = async (sessionId, paymentSyncToken, paymentReturnStatus = 'PAID') => {
    ...
    if (paymentReturnStatus && paymentReturnStatus !== 'PAID') {
      const failedApplication = await InsuranceApplication.findOneAndUpdate(
        { sessionId },
        { $set: { paymentStatus: 'FAILED', issuanceStatus: 'FAILED', paymentReturnStatus } },
        { new: true },
      );
      return { ...serializeApplicationForClient(failedApplication), syncStatus: 'FAILED' };
    }
    ...
    try {
      ...
      const pendingApplication = await InsuranceApplication.findOneAndUpdate(
        { sessionId },
        { $set: { paymentStatus: 'PENDING', issuanceStatus: 'PENDING', paymentReturnStatus: 'PAID' } },
        { new: true },
      );
      return { ...serializeApplicationForClient(pendingApplication), syncStatus: 'PENDING_CONFIRMATION' };
    }
    ...
    await notifications.insurancePaymentCompletionEmail({
      ...
      mobile:        updated?.mobile?.code && updated?.mobile?.digits
    ...
    try {
      ...
      logger.warn('Brevo updateContactAttribute failed', { email: updated.email, error: err });
    }
