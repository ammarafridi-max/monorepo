...
function normalizeCode(code) {
  ...
  if (normalized.length !== 3) throw new AppError('Currency code must be 3 characters', 400);
  return normalized;
}
...
function normalizeOptionalRate(value) {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) throw new AppError('Rate must be a positive number', 400);
  return parsed;
}
...
export function createCurrencySchema(body = {}) {
  return {
...
export function updateCurrencySchema(body = {}) {
  const payload = {};
