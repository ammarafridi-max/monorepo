...
import { Loader2, Sparkles, Plane, Plus, Trash2, UploadCloud, ArrowRight } from 'lucide-react';
...
export default function ItineraryGeneratorPage({ countries = [], onAnalytics }) {
  const { generateItinerary, isGeneratingItinerary } = useGenerateItinerary({ onAnalytics });
  ...
  const {
    ...
    defaultValues: {
      ...
      segments: [{ from: { city: '', country: '' }, to: { city: '', country: '' }, date: '' }],
    },
  });
  ...
  function handleDocs(e) {
    ...
    parseDocuments(files, {
      onSuccess: (data) => {
        const segs = (data?.segments || []).filter((s) => s.from?.city || s.to?.city || s.date);
        if (segs.length) {
          replace(
            segs.map((s) => ({
              from: { city: s.from?.city || '', country: s.from?.country || '' },
              to: { city: s.to?.city || '', country: s.to?.country || '' },
              date: s.date || '',
            })),
          );
          if (data.reservations?.flight) setValue('reservations.flight', data.reservations.flight);
          if (data.reservations?.hotel) setValue('reservations.hotel', data.reservations.hotel);
          toast.success(`Filled ${segs.length} segment${segs.length > 1 ? 's' : ''} from your documents — please review below.`);
        } else {
  ...
  function addSegment() {
    ...
    append({
      from: prev?.to?.city || prev?.to?.country ? { ...prev.to } : { city: '', country: '' },
      to: { city: '', country: '' },
      date: '',
    });
  }
  ...
  return (
    <div className="max-w-3xl mx-auto py-8">
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Create Your Travel Itinerary</h1>
        <p className="text-sm text-gray-500 mt-2 max-w-xl mx-auto">
          Tell us about your trip and we&apos;ll prepare a professional, day-by-day itinerary for your visa application.
        </p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
        {/* ---------- Part 1: details & trip ---------- */}
        <SectionCard step={1} title="Your details" subtitle="Who the itinerary is for and where you're applying.">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="First name" error={errors.traveller?.firstName}>
              <input
                className={inputClass}
                placeholder="Jane"
                {...register('traveller.firstName', { required: 'Please enter your first name' })}
              />
            </Field>
            <Field label="Email address" error={errors.traveller?.email}>
              <Controller
                ...
                rules={{ required: 'Please enter your email', pattern: { value: /^\S+@\S+\.\S+$/, message: 'Enter a valid email' } }}
                render={({ field }) => <Email email={field.value} onChange={field.onChange} />}
              />
            </Field>
            <Field label="Phone number" error={errors.traveller?.phone}>
              <Controller
                ...
                rules={{ validate: (v) => (v?.digits?.trim() ? true : 'Please enter your phone number') }}
                render={({ field }) => <PhoneInput value={field.value} onChange={field.onChange} required />}
              />
            </Field>
            <Field label="Number of travellers" error={errors.travellers}>
              <input
                ...
                max={20}
                className={inputClass}
                {...register('travellers', { min: { value: 1, message: 'At least 1 traveller' }, valueAsNumber: true })}
              />
            </Field>
            <Field label="Purpose of trip" error={errors.purpose}>
              <select className={`${inputClass} bg-white`} defaultValue="" {...register('purpose', { required: 'Select a purpose' })}>
            ...
            <Field label="Applying from (your country)" error={errors.fromCountry}>
              <CountrySelect control={control} name="fromCountry" rules={{ required: 'Select your country' }} countryItems={countryItems} />
            </Field>
            <Field label="Applying to (embassy country)" error={errors.visaCountry}>
              <CountrySelect control={control} name="visaCountry" rules={{ required: 'Select the country you are applying to' }} countryItems={countryItems} />
            </Field>
          </div>
        </SectionCard>

        {/* ---------- Part 2: supporting info ---------- */}
        <SectionCard step={2} title="Supporting information" subtitle="Optional — but it helps us craft a more accurate itinerary.">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Flight reservation">
              <select className={`${inputClass} bg-white`} {...register('reservations.flight')}>
                {RESERVATION_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </Field>
            <Field label="Hotel reservation">
              <select className={`${inputClass} bg-white`} {...register('reservations.hotel')}>
                {RESERVATION_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
            </Field>
          </div>
          ...
          <div className="mt-5">
            ...
            <label
              className={`mt-1.5 flex flex-col items-center justify-center gap-1.5 border-2 border-dashed rounded-xl px-4 py-6 transition-colors ${
                isParsing ? 'border-primary-300 bg-primary-50/40 cursor-wait' : 'border-gray-200 cursor-pointer hover:border-primary-300 hover:bg-primary-50/40'
              }`}
            >
              {isParsing ? (
              ...
              <span className="text-[11px] text-gray-400">PDF or image · we&apos;ll read these to auto-fill your trip below</span>
              <input
                type="file"
                multiple
            ...
            {docs.length > 0 && (
              <ul className="mt-2 flex flex-wrap gap-2">
                {docs.map((f, i) => (
                  <li key={i} className="text-xs text-gray-500 bg-gray-100 rounded-lg px-2.5 py-1">{f.name}</li>
                ))}
              </ul>
            )}
          </div>
        ...
        {/* ---------- Part 3: segments ---------- */}
        <SectionCard step={3} title="Your itinerary" subtitle="Add each leg of your trip with its date — e.g. Dubai → Zurich, Zurich → Paris, Paris → Dubai.">
            ...
            {fields.map((field, i) => (
              <div key={field.id} className="border border-gray-200 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {fields.length > 1 && (
                    <button type="button" onClick={() => remove(i)} className="text-gray-400 hover:text-accent-600 transition-colors" aria-label="Remove segment">
                      <Trash2 size={15} />
                    </button>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="From — city" error={errors.segments?.[i]?.from?.city}>
                    <input className={inputClass} placeholder="Dubai" {...register(`segments.${i}.from.city`, { required: 'Required' })} />
                  </Field>
                  <Field label="From — country" error={errors.segments?.[i]?.from?.country}>
                    <CountrySelect control={control} name={`segments.${i}.from.country`} rules={{ required: 'Required' }} countryItems={countryItems} />
                  </Field>
                  <Field label="To — city" error={errors.segments?.[i]?.to?.city}>
                    <input className={inputClass} placeholder="Zurich" {...register(`segments.${i}.to.city`, { required: 'Required' })} />
                  </Field>
                  <Field label="To — country" error={errors.segments?.[i]?.to?.country}>
                    <CountrySelect control={control} name={`segments.${i}.to.country`} rules={{ required: 'Required' }} countryItems={countryItems} />
                  </Field>
                  <div className="sm:col-span-2">
                    <Field label="Travel date" error={errors.segments?.[i]?.date}>
                      <Controller
                        control={control}
                        ...
                        rules={{
                          ...
                          validate: (v) => {
                            ...
                            return !prev || !v || v >= prev || 'Must be on or after the previous segment';
                          },
                        }}
                        render={({ field: f }) => (
                          <DatePicker
                            ...
                            minDate={i > 0 ? watchedSegments?.[i - 1]?.date || undefined : undefined}
                          />
                        )}
                      />
                    </Field>
      ...
      {isGeneratingItinerary && (
        <div className="fixed inset-0 z-[100] bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-sm font-medium text-gray-700">Building your day-by-day itinerary…</p>
