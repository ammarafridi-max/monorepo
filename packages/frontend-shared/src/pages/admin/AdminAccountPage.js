...
function OverviewCard({ account }) {
  const items = [
    { label: 'Role',         value: roleLabel(account?.role),      icon: BadgeCheck },
    { label: 'Status',       value: account?.status || '—',        icon: ShieldCheck },
    { label: 'Member Since', value: formatDate(account?.createdAt), icon: Clock3 },
    { label: 'Last Updated', value: formatDate(account?.updatedAt), icon: Mail },
  ];
