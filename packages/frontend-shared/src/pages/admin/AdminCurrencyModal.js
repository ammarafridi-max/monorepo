...
export default function AdminCurrencyModal({ initial, onClose, onSave, saving }) {
  ...
  function set(key, value) {
    setForm((p) => {
      ...
      if (key === 'isBaseCurrency' && value) next.rate = '1';
      return next;
    });
  }
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-1">
              {form.isBaseCurrency
                ? 'Base currency is always fixed to 1.00.'
                : 'Rate relative to the base currency.'}
            </p>
          </div>
          ...
          <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:bg-gray-50 cursor-pointer transition">
            ...
            />
            <div>
              <p className="text-xs font-semibold text-gray-700">Base currency</p>
