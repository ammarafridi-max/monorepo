...
import { getZoneByPointApi } from '../services/apiZones.js';
import { getDistanceApi } from '../services/apiLocations.js';

export const LimoBookingContext = createContext(null);

const STORAGE_KEY = 'bookingData';
const writeBooking = (value) => {
...
export function LimoBookingProvider({ children }) {
  ...
  const initialBookingData = {
    tripType: 'distance',
    bookingRef: '',
    pickup: { id: '', zone: '', name: '', address: '', lat: '', lng: '', type: '' },
    dropoff: { id: '', zone: '', name: '', address: '', lat: '', lng: '', type: '' },
    pickupDate: '',
    pickupTime: '',
    vehicle: '',
    hoursBooked: 1,
    ...
    orderSummary: {
      ...
      conversionRate: selectedCurrency?.rate,
    },
  };
  ...
  async function submitLimoForm(data) {
    try {
      ...
      const pickupZone = await getZoneByPointApi({
        lat: data.pickup.lat,
        lng: data.pickup.lng,
      });
      ...
      if (bookingData?.tripType === 'distance' && data.dropoff?.lat && data.dropoff?.lng) {
        ...
        dropoffZone = await getZoneByPointApi({
          lat: data.dropoff.lat,
          lng: data.dropoff.lng,
        });

        distance = await getDistanceApi({
      ...
      setBookingData((prev) => {
        const updated = {
          ...prev,
          pickup: { ...data.pickup, zone: pickupZone?._id || null },
          dropoff: data.dropoff ? { ...data.dropoff, zone: dropoffZone?._id || null } : null,
  ...
  function handleChange(field, value) {
    setBookingData((prev) => ({
      ...prev,
      bookingDetails: { ...prev.bookingDetails, [field]: value },
    }));
  }
  ...
  function handleNumberChange(field, value) {
    setBookingData((prev) => ({
      ...
      bookingDetails: {
        ...prev.bookingDetails,
        phoneNumber: { ...prev.bookingDetails.phoneNumber, [field]: value },
      },
    }));
  }
  ...
  function handleSelectVehicle(vehicle) {
    setBookingData((prev) => {
      const updated = {
        ...
        orderSummary: {
          ...prev.orderSummary,
          baseFare: selectedCurrency?.rate * vehicle?.totalPrice,
          currency: selectedCurrency?.code?.toLowerCase(),
          conversionRate: selectedCurrency?.rate,
        },
      };
  ...
  function handleSelectPaymentMethod(method) {
    setBookingData((prev) => {
      const updated = { ...prev, payment: { ...prev.payment, method } };
      writeBooking(updated);
      return updated;
    });
  }
  ...
  useEffect(() => {
    setBookingData((prev) => {
      ...
      return { ...prev, orderSummary: { ...prev.orderSummary, total } };
    });
  }, [
  ...
  useEffect(() => {
    setBookingData((prev) => {
      ...
      if (prev.orderSummary.conversionRate === selectedCurrency?.rate) return prev;
      const oldRate = prev.orderSummary.conversionRate || 1;

      return {
        ...prev,
        payment: { ...prev.payment, currency: selectedCurrency?.code?.toLowerCase() },
        orderSummary: {
          ...prev.orderSummary,
          baseFare: (prev.orderSummary.baseFare / oldRate) * selectedCurrency?.rate,
          currency: selectedCurrency?.code?.toLowerCase(),
          conversionRate: selectedCurrency?.rate,
        },
      };
...
export function useLimoBooking() {
  const context = useContext(LimoBookingContext);
  if (!context) {
    throw new Error('useLimoBooking must be used inside <LimoBookingProvider>');
  }
  return context;
}
