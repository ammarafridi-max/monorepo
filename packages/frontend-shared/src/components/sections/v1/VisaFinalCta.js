...
function CtaButton({ children, onClick, href, size = "md" }) {
  const sz =
    size === "lg"
      ? "text-[14px] py-2.5 px-5 sm:text-[15px] sm:py-3 sm:px-6"
      : "text-[13px] py-2 px-4 sm:text-[14px] sm:py-2.5 sm:px-5";
  const cls = `inline-flex min-h-11 items-center justify-center gap-2 font-outfit font-semibold rounded-full bg-white hover:bg-primary-50 text-primary-700 border border-white transition-colors duration-200 cursor-pointer ${sz}`;
  if (onClick)
    return (
        ...
        <div className="rounded-2xl bg-gradient-to-br from-primary-800 to-primary-950 p-8 md:p-10 flex flex-col md:flex-row items-start md:items-center gap-6 justify-between overflow-hidden relative">
          ...
          <div className="relative flex flex-col sm:flex-row gap-3 shrink-0">
            ...
            {whatsappUrl && (
              <a
                href={whatsappUrl}
                className="inline-flex min-h-11 items-center justify-center gap-2 text-[14px] py-2.5 px-5 sm:py-3 font-outfit font-semibold rounded-full border border-white/25 text-white hover:bg-white/10 transition-colors duration-200 whitespace-nowrap"
              >
                Chat on WhatsApp
              </a>
            )}
