...
export default function ShareButtons({ title, url }) {
  ...
  return (
    <>
      <p className="mb-3 text-sm font-medium text-gray-900">Share this post</p>
...
function ShareButton({ children, onClick, label }) {
  return (
    <button
      ...
      onClick={onClick}
      aria-label={label}
      className="inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg border border-gray-300 text-gray-700 transition-colors hover:bg-gray-100"
    >
      {children}
    </button>
  );
