...
import {
  ArrowLeft, Lock, Unlock, ChevronDown, ChevronUp, Plus, Trash2,
} from 'lucide-react';
...
export default function BlogForm({ initialData, onSubmit, isPending }) {
  ...
  function addFaq() {
    setValue('faqs', [...(getValues('faqs') || []), { question: '', answer: '' }]);
  }

  function removeFaq(i) {
    const current = getValues('faqs') || [];
    setValue('faqs', current.filter((_, idx) => idx !== i));
  }
        ...
        <div className="space-y-5">
                ...
                <div className="flex items-center gap-2">
                  <TextInput
                    ...
                    className={`flex-1 px-3 py-2 text-sm border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 placeholder:text-gray-300 ${
                      slugLocked ? 'bg-gray-50 text-gray-400 cursor-not-allowed border-gray-100' : 'border-gray-200'
                    }`}
                  />
                  <button
                    type="button"
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            ...
            >
              ...
              {seoOpen ? <ChevronUp size={15} className="text-gray-400" /> : <ChevronDown size={15} className="text-gray-400" />}
            </div>
            {seoOpen && (
                ...
                >
                  <TextareaInput
                    ...
                    className={`w-full px-3 py-2 text-sm border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 placeholder:text-gray-300 resize-none ${
                      metaDesc.length > 155 ? 'border-amber-300' : 'border-gray-200'
                    }`}
                  />
                </Field>
              </div>
            ...
            <Card title="Post Info">
              <dl className="space-y-2 text-xs">
                {[
                  ...
                  ['Created',   initialData.createdAt ? format(new Date(initialData.createdAt), 'dd MMM yyyy') : '—'],
                  ['Updated',   initialData.updatedAt ? format(new Date(initialData.updatedAt), 'dd MMM yyyy') : '—'],
                ].map(([k, v]) => (
            ...
            <div className="space-y-2">
              {['draft', 'published', 'scheduled'].map((s) => (
                <label key={s} className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-xs font-semibold text-gray-600 capitalize">{s}</span>
                </label>
              ))}
            </div>
            {status === 'scheduled' && (
