...
export default function MegaMobileNavigation({ pages = [], logoAlt = "Logo" }) {
  ...
  return (
    <nav className="block lg:hidden absolute top-0 left-0 right-0 z-9999 bg-transparent py-3">
      <Container className="flex justify-between items-center">
        <Link href="/" className="w-15 h-auto flex items-center">
          <Image
            src="/logo.webp"
            alt={logoAlt}
            width={100}
                        ...
                        <div className="mt-1 mb-2 flex flex-col rounded-xl bg-gray-50 p-1.5">
                          {page.subpages.map((sub, idx) => (
                            <Link
                              ...
                              className="flex items-start gap-3 px-4 py-2.5 rounded-lg hover:bg-gray-100"
                            >
                              {sub.description && (
                                <span className="w-1.5 h-1.5 rounded-full bg-primary-400 shrink-0 mt-[6px]" />
                              )}
                              <div className="min-w-0">
                                <p className="font-medium text-sm text-gray-700 leading-snug">
                                  {sub.name}
                                </p>
                                {sub.description && (
