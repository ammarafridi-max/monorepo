...
import { connectMongo, AdminUser, Order, ORDER_STATES } from '@picturesk/shared';
...
before(async () => {
  ...
  await AdminUser.create([
    { name: 'Admin', username: 'adminuser1', email: 'admin@p.ai', passwordHash: await hashPassword('supersecret'), role: 'admin', status: 'ACTIVE' },
    { name: 'Support', username: 'supportone', email: 'support@p.ai', passwordHash: await hashPassword('supersecret'), role: 'support', status: 'ACTIVE' },
  ]);
...
test('resend-email: 503 when no email client is configured', async () => {
  ...
  const { router: authRouter, protect } = createAdminAuth(CONFIG);
  app.use('/auth', authRouter);
  app.use('/admin', createAdminActionsRouter({ guard: protect, restrictTo, stripe, orderPipeline: queue, pipelineJobOpts, emailClient: null, webBaseUrl: 'http://x' }));
