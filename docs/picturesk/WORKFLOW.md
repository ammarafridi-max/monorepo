...
The admin panel is a separate, staff-only area. Distinct identity (the shared
`admin-user` model), distinct cookie (`jwt`, set by the api on its own origin),
its own login. Two roles: **admin** (full access) and **support** (read-only).
